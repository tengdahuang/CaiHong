...
namespace HSEMS.Bll
{
    public class BllEquipmentProductHandle
    {
        public static long GetCount(string optrType)
        {
            return EquipmentMaintenance.GetCount(CK.K["OptrType"] == optrType && CK.K["Status"] == EquipmentResult.产品处理.ToString());
        }

        public static DataTable GetTable(string optrType)
        {

            var dt = EquipmentMaintenance.Find(CK.K["OptrType"] == optrType && CK.K["Status"] == EquipmentResult.产品处理.ToString()).ToDataTable();
