...
namespace HSEMS.Bll
{
    public class BllEquipmentVariationHandle
    {
        public static long GetCount(string optrType)
        {
            return EquipmentVariation.GetCount(CK.K["OptrType"] == optrType && CK.K["Status"] == VariationResult.已接单.ToString());
        }

        public static DataTable GetTable(string optrType)
        {

            var dt = EquipmentVariation.Find(CK.K["OptrType"] == optrType && CK.K["Status"] == VariationResult.已接单.ToString()).ToDataTable();
