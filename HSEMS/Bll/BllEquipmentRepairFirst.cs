...
namespace HSEMS.Bll
{
    public class BllEquipmentRepairFirst
    {

        public static long GetCount(string optrType)
        {
            return EquipmentMaintenance.GetCount(CK.K["OptrType"] == optrType && CK.K["Status"] == EquipmentResult.已接单.ToString());
        }

        public static DataTable GetTable(string optrType)
        {

            var dt = EquipmentMaintenance.Find(CK.K["OptrType"] == optrType && CK.K["Status"] == EquipmentResult.已接单.ToString()).ToDataTable();
